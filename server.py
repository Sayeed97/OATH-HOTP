...
class User(db.Model):
    ...
    reset = db.Column(db.Integer, default=0)            # 1 if secret needs reset, else 0
...
def user_page():
    if request.method == "POST":
        ...
        user.reset = 0  # reset by default

        # If the token and server counters are in sync and OTP matches
        # Logic = Exact match then increment server counter
        if hotp.verify(otp_input, server_counter):
            ...
            # Token and Server OTP don't match then server looks ahead 5
            # Logic = Look 1 to 5 steps ahead
            found_drift = None
            for i in range(1, 6):
                if hotp.verify(otp_input, server_counter + i):
                    found_drift = i # This means the token has drifted n counters from the server
                    break

            # This indicates that server needs to be incremented by n drifted counters
            if found_drift:
            ...
            # This indicates that the server and token are completely out of sync
            # In order to re-initiate the user the admin must reset the token and server
            else:
                user.drift = 0
                user.reset = 1
